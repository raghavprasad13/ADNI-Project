...
test_data = atlas_df['Test'].tolist()

TEST_INDICES = {'MMSE': 0, 'NPI-Q': 1, 'Both': 2}
u_color = ['green', 'red', 'blue']
color = [u_color[TEST_INDICES[test]] for test in test_data]
# print(loc_mat.shape)

cn_s_obj = SourceObj('rois', loc_mat, data=percolation_states['CN'], color='green', alpha=0.5, edge_width=1., radius_min=2., radius_max=15.)
ad_s_obj = SourceObj('rois', loc_mat, data=np.arange(len(test_data)), color=color, alpha=0.5, edge_width=1., radius_min=10, radius_max=10)
...
c_obj = ConnectObj('edges', loc_mat, adj_mat,
                   dynamic=(.1, 1.), cmap='gnuplot', antialias=True,
                   dynamic_order=1)

# func_net = nx.from_numpy_array(adj_mat)
# print(func_net.number_of_edges())
vb = Brain(source_obj=ad_s_obj, connect_obj=None)
